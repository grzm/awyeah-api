...
(ns com.grzm.awyeah.protocols-test
  ...
  (:require
   ...
   [com.grzm.awyeah.protocols.ec2]
   [com.grzm.awyeah.protocols.json]
   [com.grzm.awyeah.protocols.query]
   [com.grzm.awyeah.protocols.rest-json]
...
(defn instrument-fixture [f]
  (try
    (stest/instrument ['com.grzm.awyeah.util/query-string])
    (f)
    (finally
      (stest/unstrument))))
...
(defn test-protocol
  ...
  ([protocol input-or-output]
   (let [filepath       (str "botocore/protocols/" input-or-output "/" protocol ".json")
     ...
     (doseq [test (into (-> filepath io/resource slurp (json/read-str))
                        (when (io/resource extra-filepath)
                          (-> extra-filepath io/resource slurp (json/read-str))))]
