...
(ns ^:skip-wiki com.grzm.awyeah.shape
...
(defn- parse-date* [d & formats]
  (->> formats
       (map #(try (util/parse-date % d) (catch java.time.format.DateTimeParseException _ nil)))
       (filter identity)
       first))
...
(defn json-parse
  ...
  [shape s]
  (json-parse* shape (json/read-str s)))
...
(defmethod xml-parse* "boolean" [_shape nodes] (= (content nodes) "true"))
